...
def half_circle_barrage(x, y, tag_x, tag_y, size=1, speed=1):  # tag为玩家坐标
    ...
    if tag_x-x < tag_y-y and tag_x-x > y-tag_y:
        if tag_x-x > 0:
            ...
            barrage_list.append(barrage(x, y, -speed, 0, size))
            barrage_list.append(
                barrage(x, y, -0.707*speed, -0.707*speed, size))
            barrage_list.append(barrage(x, y, -0.707*speed, 0.707*speed, size))
            barrage_list.append(barrage(x, y, 0, speed, size))
            barrage_list.append(barrage(x, y, 0, -speed, size))
    else:
...
def ZPW(difficulty):
    ...
    while p.hp > 0:
        if time.ticks_diff(time.ticks_ms(), update_time) > (60 // fps):
            ...
            if p.score >= 50:  # 50分获胜
                show_win()
            if sound.read() > 1300 and p.mp >= 500:
                skill_trigger(p, 3)
            if sound.read() > 800 and p.mp >= 400:
                skill_trigger(p, 2)
            if sound.read() > 300 and p.mp >= 300:
                skill_trigger(p, 1)
            update_time = time.ticks_ms()
    if p.hp <= 0:
        show_death()
