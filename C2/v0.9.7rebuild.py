...
def half_circle_barrage(x, y, tag_x, tag_y, size=1, speed=1):  # tag为玩家坐标
    global barrage_list
    if tag_x-x <tag_y-y and tag_x-x>y-tag_y:
        if tag_x-x>0:
        ...
        if tag_y-y>0:
            barrage_list.append(barrage(x,y0,0,speed,size))
            barrage_list.append(barrage(x,y,0.707*speed,0.707*speed,size))
            barrage_list.append(barrage(x,y,-0.707*speed,0.707*speed,size))
            barrage_list.append(barrage(x,y,speed,0,size))
            barrage_list.append(barrage(x,y,-speed,0,size))
...
def wh_circle_barrage(x, y, tag_x, tag_y, size=1, speed=1):
    ...
    oth_deg_barrage(x, y, 2*x-tag_x, 2*y-tag_y, size, speed)
...
def seed2seed(aseed):
    aseed[0] = (int(aseed[0])+2*int(aseed[3])+3*int(aseed[6])) % 10
    aseed[1] = (int(aseed[1])+2*int(aseed[4])+3*int(aseed[7])) % 10
    aseed[2] = (int(aseed[2])+2*int(aseed[5])+3*int(aseed[8])) % 10
    aseed[3] = (int(aseed[3])+2*int(aseed[6])+3*int(aseed[9])) % 10
    aseed[4] = (int(aseed[4])+2*int(aseed[7])+3*int(aseed[0])) % 10
    aseed[5] = (int(aseed[5])+2*int(aseed[8])+3*int(aseed[1])) % 10
    aseed[6] = (int(aseed[6])+2*int(aseed[9])+3*int(aseed[2])) % 10
    aseed[7] = (int(aseed[7])+2*int(aseed[0])+3*int(aseed[3])) % 10
    aseed[8] = (int(aseed[8])+2*int(aseed[1])+3*int(aseed[4])) % 10
    aseed[9] = (int(aseed[9])+2*int(aseed[2])+3*int(aseed[5])) % 10
    return aseed
...
def ZPW(result):
    ...
    while p.hp > 0:
        ...
        if round(get_time())>barstep:
            barstep+=3
            astep = result.pop() if len(result)>0 else []
        else:
            astep=[]
        player_ui(p, astep)
