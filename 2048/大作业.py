class Player():
    ...
    def switch(self,my_board,direction,chessman):
        def changeformat(mybls):
            if len(mybls)==self.ROWS*self.COLLUMS:
                pass
            else:
                pass
        movedict = {
        ...
        if direction in [0,2]:   # 0,1,2,3  ->  上下左右
            ...
            #for i in range(len(lst)):
            #    print(lst[i])
            return bytearray(lst[i][j] for j in range(self.COLLUMS)\
                for i in range(self.ROWS))
         

    def move_row(self,mybyte,side):
            protectnum = []
            def move_one(i,mybyte,side):
                isFriend = mybyte[i] // 0x80
                #if self.isFirst != (isFriend==0):
                #    return
                if isFriend != 0:
                    return
                while i > 0:
    ...
    def move_up(self,lst,chessman):
        for i in range(len(lst)):
            if (self.where == 1 and i < self.COLLUMS//2) or\
               (self.where == 0 and i >= self.COLLUMS//2):  #  1代表我方在左
                lst[i] = self.move_row(lst[i],'')
            else:
                lst[i] = self.move_row(lst[i],'change')
        return lst
    ...
    def move_left(self,lst):
        if self.where == 1:   # 我方在左
            for i in range(len(lst)):
                lst[i] = self.move_row(lst[i],'left')
        else:
            for i in range(len(lst)):
                lst[i] = self.move_row(lst[i],'right')
        return lst
 
    def move_right(self,lst):
        if self.where == 0:   # 倒置后我方在左
            for i in range(len(lst)):
                lst[i].reverse()
                lst[i] = self.move_row(lst[i],'left')
                lst[i].reverse()
...
test3 = bytearray(         [0,1,1,1,0x80,128,128,128,
                            ...
                            [i*8+j] for j in range(8) for i in range(4))
test4 = bytearray(         [0,1,2,3,128,129,130,131,
                            0,1,2,3,131,3,131,128,
                            0,1,2,3,128,129,130,131,
                            0,0,0,129,1,128,128,128 ]
                            [i*8+j] for j in range(8) for i in range(4))

outcome = Player(True , 1).switch(test4,1)
